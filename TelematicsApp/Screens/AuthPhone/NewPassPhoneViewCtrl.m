...
@interface NewPassPhoneViewCtrl () <UITextFieldDelegate> {
    int currSeconds;
}
...
@implementation NewPassPhoneViewCtrl
...
- (void)viewDidLoad {
    ...
    [self.signInBtn.layer setCornerRadius:15.0f];
    ...
    [self.signInBtn setAttributedTitle:nextText forState:UIControlStateNormal];
...
//                [self errorPhoneFunctionAfterResetPassword];
//                return;
//            } else {
//                [self showPreloader];
...
- (void)errorNewPassFunctionInvalidCode {
    ...
    self.userDesc.text = localizeString(@"validation_invalid_code_check_phone");
    self.userDesc.textColor = [Color officialRedColor];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4. * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (IS_IPHONE_5 || IS_IPHONE_4)
            self.userDesc.font = [Font regular12];
...
- (void)errorPhoneFunctionAfterResetPassword {
    if (IS_IPHONE_5 || IS_IPHONE_4)
        self.userDesc.font = [Font medium10];
    
    self.userDesc.text = localizeString(@"An error occurred while changing the password. Try again please");
    self.userDesc.textColor = [Color officialRedColor];
    [self.signInBtn setTitle:localizeString(@"TRY AGAIN") forState:UIControlStateNormal];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4. * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (IS_IPHONE_5 || IS_IPHONE_4)
            self.userDesc.font = [Font regular12];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField == self.createPassField) {
        [self.view endEditing:YES];
        [self nextBtnAction:self.signInBtn];
        return YES;
    }
    return YES;
}
