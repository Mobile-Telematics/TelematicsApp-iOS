...
- (void)viewDidLoad {
    ...
    if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleDot) {
        ...
        _pageLabel.textColor = [Color officialWhiteColor];
...
- (void)showDismissalAnimation {
    ...
    if (_bounces) {
        [UIView animateWithDuration:kSpringAnimationDuration delay:0 usingSpringWithDamping:1.0 initialSpringVelocity:0 options:kNilOptions animations:^{
            photoView.imageView.frame = sourceRect;
            self.view.backgroundColor = [UIColor clearColor];
            self.backgroundView.alpha = 0;
        } completion:^(BOOL finished) {
