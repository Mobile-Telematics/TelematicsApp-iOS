...
@implementation TowingSwitch

- (instancetype)init
{
    self = [super init];
    if (self) {
        [NSException raise:@"TowingSwitchException" format:@"Init call is prohibited, use initWithStringsArray: method"];
    }
...
{
    return [[TowingSwitch alloc] initWithStringsArray:strings];
}
...
- (instancetype)initWithStringsArray:(NSArray *)strings
{
    ...
    self.sliderColor = [Color officialWhiteColor];
    self.labelTextColorInsideSlider = [UIColor blackColor];
    self.labelTextColorOutsideSlider = [Color officialWhiteColor];
