...
@interface ScoreAccVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Acceleration")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardAcceleration.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardAcceleration = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.AccelerationPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.AccelerationPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.AccelerationPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.AccelerationPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.AccelerationPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardAcceleration.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardAcceleration[indexPath.row];
    if (!cellGlobal) {
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.AccelerationPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardAcceleration.count-1) && userProperties.AccelerationPlace.intValue != 11 && userProperties.AccelerationPlace.intValue != 12 && userProperties.AccelerationPlace.intValue != 13 && userProperties.AccelerationPlace.intValue != 14 && userProperties.AccelerationPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_acceleration"];
