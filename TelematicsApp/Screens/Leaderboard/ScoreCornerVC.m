...
@interface ScoreCornerVC () <UITableViewDelegate, UITableViewDataSource>
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.TurnPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.TurnPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.TurnPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.TurnPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.TurnPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardTurn.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardTurn[indexPath.row];
    if (!cellGlobal) {
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.TurnPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardTurn.count-1) && userProperties.TurnPlace.intValue != 11 && userProperties.TurnPlace.intValue != 12 && userProperties.TurnPlace.intValue != 13 && userProperties.TurnPlace.intValue != 14 && userProperties.TurnPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_cornering"];
