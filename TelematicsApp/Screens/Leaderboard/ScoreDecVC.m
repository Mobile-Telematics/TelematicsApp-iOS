...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Braking")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardDeceleration.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardDeceleration = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.DecelerationPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.DecelerationPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.DecelerationPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.DecelerationPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.DecelerationPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardDeceleration.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardDeceleration[indexPath.row];
    if (!cellGlobal) {
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.DecelerationPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardDeceleration.count-1) && userProperties.DecelerationPlace.intValue != 11  && userProperties.DecelerationPlace.intValue != 12 && userProperties.DecelerationPlace.intValue != 13 && userProperties.DecelerationPlace.intValue != 14 && userProperties.DecelerationPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_deceleration"];
