...
@interface ScoreMileageVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Mileage")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardDistance.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardDistance = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.DistancePlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.DistancePlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.DistancePlace.intValue <= 11) {
            return 11;
        } else if (userProperties.DistancePlace.intValue <= 12) {
            return 12;
        } else if (userProperties.DistancePlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardDistance.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardDistance[indexPath.row];
    if (!cellGlobal) {
    ...
    float distance = objGlobal.Value.floatValue;
    if ([Configurator sharedInstance].needDistanceInMiles || [defaults_object(@"needDistanceInMiles") boolValue]) {
        distance = convertKmToMiles(objGlobal.Value.floatValue);
    }
    cellGlobal.scoreLbl.text = [NSString stringWithFormat:@"%.1f", distance];
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.DistancePlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardDistance.count-1) && userProperties.DistancePlace.intValue != 11 && userProperties.DistancePlace.intValue != 12 && userProperties.DistancePlace.intValue != 13 && userProperties.DistancePlace.intValue != 14 && userProperties.DistancePlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_mileage"];
