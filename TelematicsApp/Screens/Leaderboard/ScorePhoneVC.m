...
@interface ScorePhoneVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Distraction")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardDistraction.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardDistraction = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.DistractionPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.DistractionPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.DistractionPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.DistractionPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.DistractionPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardDistraction.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardDistraction[indexPath.row];
    if (!cellGlobal) {
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.DistractionPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardDistraction.count-1) && userProperties.DistractionPlace.intValue != 11 && userProperties.DistractionPlace.intValue != 12 && userProperties.DistractionPlace.intValue != 13 && userProperties.DistractionPlace.intValue != 14 && userProperties.DistractionPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_phone"];
