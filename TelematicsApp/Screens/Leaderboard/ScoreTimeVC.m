...
@interface ScoreTimeVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Time Driven")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardDuration.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardDuration = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.DurationPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.DurationPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.DurationPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.DurationPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.DurationPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardDuration.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardDuration[indexPath.row];
    if (!cellGlobal) {
    ...
    float hours = objGlobal.Value.floatValue/60;
    
    cellGlobal.scoreLbl.text = [NSString stringWithFormat:@"%.1f", hours];
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.DurationPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardDuration.count-1) && userProperties.DurationPlace.intValue != 11 && userProperties.DurationPlace.intValue != 12 && userProperties.DurationPlace.intValue != 13 && userProperties.DurationPlace.intValue != 14 && userProperties.DurationPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_time"];
