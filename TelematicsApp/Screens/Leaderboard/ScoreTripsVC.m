...
@interface ScoreTripsVC () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    self.scoreTitle.attributedText = [self createLabelImgBefore:localizeString(@"Total Trips")];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.leaderboardModel.leaderboardTrips.count == 0) {
        [[MainApiRequest requestWithCompletion:^(id response, NSError *error) {
            if (!error && [response isSuccesful]) {
                self.leaderboard = response;
                self.leaderboardModel.leaderboardTrips = self.leaderboard.Result.Users;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (userProperties != nil) {
        if (userProperties.TripsPlace.intValue <= 10 && userProperties.UsersNumber.intValue <= 10) {
            return userProperties.UsersNumber.intValue;
        } else if (userProperties.TripsPlace.intValue <= 10 && userProperties.UsersNumber.intValue > 10) {
            return 10;
        } else if (userProperties.TripsPlace.intValue <= 11) {
            return 11;
        } else if (userProperties.TripsPlace.intValue <= 12) {
            return 12;
        } else if (userProperties.TripsPlace.intValue <= 13) {
            return 13;
        } else {
            return self.leaderboardModel.leaderboardTrips.count;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeaderboardObject *objGlobal = self.leaderboardModel.leaderboardTrips[indexPath.row];
    if (!cellGlobal) {
    ...
    cellGlobal.scoreLbl.text = [NSString stringWithFormat:@"%.0f", objGlobal.Value.floatValue];
    ...
    if (objGlobal.IsCurrentUser.intValue == 1) {
        ...
        if (userProperties.TripsPlace.intValue >= 11) {
            if ((indexPath.row == 10 || indexPath.row == self.leaderboardModel.leaderboardTrips.count-1) && userProperties.TripsPlace.intValue != 11 && userProperties.TripsPlace.intValue != 12 && userProperties.TripsPlace.intValue != 13 && userProperties.TripsPlace.intValue != 14 && userProperties.TripsPlace.intValue != 15) {
...
- (NSMutableAttributedString*)createLabelImgBefore:(NSString*)text {
    ...
    imageAttachment.image = [UIImage imageNamed:@"lead_trips"];
