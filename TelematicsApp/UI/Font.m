//
//  Font.m
//  TelematicsApp
//
//  Created by DATA MOTION PTE. LTD. on 24.01.21.
//  Copyright © 2021 DATA MOTION PTE. LTD. All rights reserved.
//

#import "Font.h"

@implementation Font

+ (UIFont*)light8Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Light" size:8];
}

+ (UIFont*)light10Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Light" size:10];
}

+ (UIFont*)regular12Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue" size:12];
}

+ (UIFont*)regular13Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue" size:13];
}

+ (UIFont*)medium12Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Medium" size:12];
}

+ (UIFont*)medium15Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Medium" size:15];
}

+ (UIFont*)medium18Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Medium" size:18];
}

+ (UIFont*)bold18Helvetica {
    return [UIFont fontWithName:@"HelveticaNeue-Bold" size:18];
}

+ (UIFont*)thin11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightThin];
}

+ (UIFont*)light9 {
    return [UIFont systemFontOfSize:9.0f weight:UIFontWeightLight];
}

+ (UIFont*)light10 {
    return [UIFont systemFontOfSize:10.0f weight:UIFontWeightLight];
}

+ (UIFont*)light11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightLight];
}

+ (UIFont*)light12 {
    return [UIFont systemFontOfSize:12.0f weight:UIFontWeightLight];
}

+ (UIFont*)light13 {
    return [UIFont systemFontOfSize:13.0f weight:UIFontWeightLight];
}

+ (UIFont*)regular9 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular12 {
    return [UIFont systemFontOfSize:12.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular13 {
    return [UIFont systemFontOfSize:13.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular14 {
    return [UIFont systemFontOfSize:14.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular15 {
    return [UIFont systemFontOfSize:15.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular16 {
    return [UIFont systemFontOfSize:16.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular17 {
    return [UIFont systemFontOfSize:17.0f weight:UIFontWeightRegular];
}

+ (UIFont*)regular19 {
    return [UIFont systemFontOfSize:19.0f weight:UIFontWeightRegular];
}

+ (UIFont*)medium10 {
    return [UIFont systemFontOfSize:10.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium12 {
    return [UIFont systemFontOfSize:12.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium13 {
    return [UIFont systemFontOfSize:13.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium14 {
    return [UIFont systemFontOfSize:14.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium15 {
    return [UIFont systemFontOfSize:14.0f weight:UIFontWeightMedium];
}

+ (UIFont*)medium17 {
    return [UIFont systemFontOfSize:17.0f weight:UIFontWeightMedium];
}

+ (UIFont*)semibold7 {
    return [UIFont systemFontOfSize:7.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold10 {
    return [UIFont systemFontOfSize:10.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold13 {
    return [UIFont systemFontOfSize:13.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold15 {
    return [UIFont systemFontOfSize:15.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold17 {
    return [UIFont systemFontOfSize:17.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold18 {
    return [UIFont systemFontOfSize:18.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold22 {
    return [UIFont systemFontOfSize:22.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)semibold23 {
    return [UIFont systemFontOfSize:22.0f weight:UIFontWeightSemibold];
}

+ (UIFont*)bold11 {
    return [UIFont systemFontOfSize:11.0f weight:UIFontWeightBold];
}

+ (UIFont*)bold12 {
    return [UIFont systemFontOfSize:12.0f weight:UIFontWeightBold];
}

+ (UIFont*)bold13 {
    return [UIFont systemFontOfSize:12.0f weight:UIFontWeightBold];
}

+ (UIFont*)bold14 {
    return [UIFont boldSystemFontOfSize:14];
}

+ (UIFont*)bold15 {
    return [UIFont boldSystemFontOfSize:15];
}

+ (UIFont*)bold17 {
    return [UIFont boldSystemFontOfSize:17];
}

+ (UIFont*)heavy14 {
    return [UIFont systemFontOfSize:14.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy16 {
    return [UIFont systemFontOfSize:16.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy17 {
    return [UIFont systemFontOfSize:16.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy18 {
    return [UIFont systemFontOfSize:18.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy20 {
    return [UIFont systemFontOfSize:20.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy21 {
    return [UIFont systemFontOfSize:21.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy24 {
    return [UIFont systemFontOfSize:24.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy26 {
    return [UIFont systemFontOfSize:26.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy44 {
    return [UIFont systemFontOfSize:44.0f weight:UIFontWeightHeavy];
}

+ (UIFont*)heavy54 {
    return [UIFont systemFontOfSize:54.0f weight:UIFontWeightHeavy];
}

@end
